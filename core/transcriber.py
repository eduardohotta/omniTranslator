...
class VoskEngine:
    ...
    def _initialize(self):
        ...
        if not self.model:
            print(f"Vosk Init Error: Failed to load model from {self.model_path} or subfolders.")


    def process_audio(self, audio_bytes, is_speech):
        if not self.recognizer: return None, None
        
        if not hasattr(self, "silence_frames"): self.silence_frames = 0
...
class GoogleEngine:
    def __init__(self, sample_rate):
        ...
        self.silence_frames = 0
        self.silence_threshold_frames = 1 # ~30ms (instant trigger after AudioCapture says ok)
        self.thinking = False
    ...
    def process_audio(self, audio_bytes, is_speech):
        ...
        trigger = (self.silence_frames >= self.silence_threshold_frames and len(self.buffer) > 0)
        ...
        if (trigger or safety_trigger):
            ...
            print(f"LOG: Segmento pronto para reconhecimento ({duration_s:.2f}s). Enviando para Google...")
    ...
    def recognize(self, audio_data_bytes):
        ...
        try:
            import numpy as np
            audio_np = np.frombuffer(audio_data_bytes, dtype=np.int16).astype(np.float32)
            ...
            if max_val > 0:
                audio_np = (audio_np / max_val) * 30000.0 # Normalize to near-peak 16-bit
