...
def main():
    ...
    def apply_restart():
        ...
        window.update_text(
            "Carregando...", f"Iniciando motor {m_type}...", to_history=False
        )
    ...
    def on_load_finished(new_transcriber, new_translator):
        ...
        window.update_text(
            "Concluído", "Modelo carregado com sucesso.", to_history=False
        )
        thread.resume_audio()
    ...
    try:
        ...
        audio = AudioCapture(device_index=dev_idx, energy_threshold=vad_th)

        translator = None
        if HAS_TRANSLATOR:
            try:
                translator = Translator(
                    from_code="pt", to_code=config.get("target_lang", "en")
                )
            except Exception as e:
        ...
        if "audio" not in locals():
            audio = AudioCapture(device_index=config.get("audio_device_index"))
        if "transcriber" not in locals():
            transcriber = Transcriber("missing")
